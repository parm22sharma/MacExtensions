...
#if !os( macOS )
// helper function
func SCNVector3ToFloat3(_ scnVector: SCNVector3 ) -> float3 {
    return float3( x: scnVector.x, y: scnVector.y, z: scnVector.z )
}
// helper function
func SCNVector3FromFloat3(_ float: float3 ) -> SCNVector3 {
    return SCNVector3( x: float.x, y: float.y, z: float.z )
}
// helper function
// swiftlint:disable line_length
func SCNMatrix4FromMat4( _ matrix: matrix_float4x4 ) -> SCNMatrix4 {
    return SCNMatrix4(m11: matrix.columns.0.x, m12: matrix.columns.0.y, m13: matrix.columns.0.z, m14: matrix.columns.0.w, m21: matrix.columns.1.x, m22: matrix.columns.1.y, m23: matrix.columns.1.z, m24: matrix.columns.1.w, m31: matrix.columns.2.x, m32: matrix.columns.2.y, m33: matrix.columns.2.z, m34: matrix.columns.2.w, m41: matrix.columns.3.x, m42: matrix.columns.3.y, m43: matrix.columns.3.z, m44: matrix.columns.3.w)
}
...
class Character: NSObject {
    ...
    func update(atTime time: TimeInterval, with renderer: SCNSceneRenderer) {
        ...
        let hitFrom = SCNVector3FromFloat3(p0)
        let hitTo = SCNVector3FromFloat3(p1)
        ...
        if let hit = hitResult {
            let ground = SCNVector3ToFloat3(hit.worldCoordinates)
    ...
    func slideInWorld(fromPosition start: float3, velocity: float3) {
        ...
        while !stop {
            ...
            let contacts = physicsWorld!.convexSweepTest(
                ...
                from: SCNMatrix4FromMat4(from),
                to: SCNMatrix4FromMat4(to),
                options: options)
            if !contacts.isEmpty {
    ...
    private func handleSlidingAtContact(_ closestContact: SCNPhysicsContact, position start: float3, velocity: float3)
        ...
        let slidePlaneNormal = SCNVector3ToFloat3(closestContact.contactNormal)
        let slidePlaneOrigin = SCNVector3ToFloat3(closestContact.contactPoint)
